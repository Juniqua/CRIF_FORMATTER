...
f_i_code = "AHLAL" #@param {type:"string"}
branch_code = "BRANCH01" #@param {type:"string"}
last_acc_date = "31032022" #@param {type:"string"}

code = "007" #@param {type:"string"}
corr_flag = "  " #@param {type:"string"}

#INPUT CAN BE ENTERED MANUALLY IN THIS ORDER: 'f_i_code, branch_code, last_acc_date, code, corr_flag'
INP = 'f_i_code, branch_code, last_acc_date, code, corr_flag'
#ALSO INPUT CAN BE EQUALLED TO THE VARIABLES PLACED IN THE FORM ABOVE
INP = f_i_code, branch_code, last_acc_date, code, corr_flag

def parser(input_,sUBJECT_FILE,CONTRACT_FILE):
  #@title Enter the information below in the correct format per field. { vertical-output: true, form-width: "50%", display-mode: "form" }
  import datetime as dt
  today = dt.date.today()
  ...
  err_df.to_excel('payment_error_file.xlsx')
  #SENDS A LIST OF THE NEW PAYMENTS NUMBERS, CORRESPONDS LINE FOR LINE ON 'ERR_DF' FILE
  pmts_fix.to_excel('payment_error_fixes.xlsx')
  ...
  date_and_code = str(filedate +'_'+f_i_code+'.zip')
...
parser(INP,'subject.xlsx','contract.xlsx')
from flask import Flask, request, render_template, jsonify, json, session, url_for, redirect,send_file,send_from_directory,abort,safe_join
...
from wtforms import SubmitField, FileField, StringField,TextAreaField,DateField
...
ALLOWED_EXTENSIONS = {'csv', 'xlsx', 'gsheet'}
...
app.config['UPLOAD_FOLDER']= 'C://Users/Juniqua/Desktop/deploy/uploads'
app.config['DOWNLOAD_FOLDER']='C://Users/Juniqua/Desktop/deploy/downloads'
#f = open('parser.pkl','rb')
#parser = pickle.load(f,encoding='bytes')


class crifForm(FlaskForm):
  f_i_code = StringField("Financial Institution Code - XXXXX",validators=[DataRequired()])
  branch_code = StringField("Branch Code - BRANCH00",validators=[DataRequired()])
  last_acc_date = DateField("Last Accounting Date - 00000000",validators=[DataRequired()], format='%d%m%Y')
  date_of_prod = DateField("Production Date - 00000000",validators=[DataRequired()], format='%d%m%Y')
  code = StringField("Code - 000",validators=[DataRequired()])
  corr_flag = StringField("Correction flag - '1' or ' '",validators=[DataRequired()])
  contract_columns = FileField("Submit 'CRIFCONTRACTDATA.xlsx' File Here",validators=[DataRequired()])
  subject_columns = FileField("Submit 'CRIFSUBJECTDATA.xlsx' File Here",validators=[DataRequired()])
...
#@bp.route('/format_complete',methods=['GET','POST']) 
@app.route('/format_complete',methods=['GET']) 
def format_complete():
    #form = crifForm()
    if request.method == 'POST':
        f = request.files['subject_columns']
        f.save(os.path.join(app.config['UPLOAD_FOLDER'], secure_filename('subject_columns.xlsx')))
        f = request.files['contract_columns']
        f.save(os.path.join(app.config['UPLOAD_FOLDER'], secure_filename('contract_columns.xlsx')))

    data = request.form


    date_of_prod = data('date_of_prod')
    last_acc_date = data('last_acc_date')
    f_i_code = data('f_i_code')
    code = data('code')
    branch_code = data('branch_code')
    corr_flag = data('corr_flag')

    input_ = f_i_code, branch_code, last_acc_date, date_of_prod,code, corr_flag
    ...
    parser(input_,'uploads/subject_columns.xlsx','uploads/contract_columns.xlsx')

    dnc = str(str(date_of_prod) +'_'+str(f_i_code))
    date_and_code = (dnc +'.zip')
    zzip_file = dnc
    ...
    for zf in output_filename:
        shutil.make_archive(zf, 'zip', input_dir)
        return
        #Delete the zip file if not needed
        #os.remove("completed_file.zip")

    result_file = send_from_directory(app.config['DOWNLOAD_FOLDER'],date_and_code)
    
    return render_template('format_complete', results = result_file)

@app.route('/get-zipfile/<path:path>',methods = ['GET','POST'])
